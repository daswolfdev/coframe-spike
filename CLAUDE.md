...
- **real-healthcheck** — no service ships an always-true placeholder probe
  (`true`//bin/true in any YAML spelling, checked before `git add` too); the
  service contract's "real healthcheck" rule is enforced, not reviewed.
